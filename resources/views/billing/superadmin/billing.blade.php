...
@section('title', 'Configuración de Facturación del Sistema (SuperAdmin)')
@section('page-title', 'Facturación del Sistema')
@section('page-subtitle', 'Configura los parámetros y firma electrónica para las facturas emitidas a los clientes de la plataforma SaaS')
...
<div class="mt-2 max-w-5xl mx-auto page-fade space-y-6">
    ...
    <div class="mb-4">
        <a href="{{ route('superadmin.dashboard') }}" class="btn-outline">
            <i class="fa-solid fa-arrow-left"></i> Volver a Panel SuperAdmin
        </a>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-1 space-y-6">
            <div class="card-panel p-6 space-y-4">
                <h3 class="text-sm font-bold border-b pb-2" style="color: var(--text-main); border-color: var(--border-light);">Facturación de la Plataforma</h3>
                <p class="text-xs text-gray-500 leading-relaxed">
                    Estos parámetros se utilizan para facturar de manera oficial las suscripciones mensuales/anuales aprobadas para los distintos inquilinos del sistema.
                </p>
            </div>
        </div>
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="card-panel p-6 space-y-4">
                <h3 class="text-sm font-bold border-b pb-2" style="color: var(--text-main); border-color: var(--border-light);">Listado de Firmas Electrónicas de la Plataforma</h3>

                @if($certificates->isEmpty())
                    <div class="text-center py-6 text-xs text-gray-500">
                        <i class="fa-solid fa-file-signature text-3xl mb-2 text-gray-400"></i>
                        <p>No hay firmas configuradas para el SuperAdmin aún.</p>
                    </div>
                @else
                                ...
                                @foreach($certificates as $cert)
                                    <tr class="border-b last:border-b-0 hover:bg-slate-50 dark:hover:bg-slate-800/20" style="border-color: var(--border-light);">
                                        ...
                                        <td class="py-3 text-center">
                                            ...
                                            @else
                                                <form method="POST" action="{{ route('superadmin.billing.certificates.default', $cert->id) }}" class="inline">
                                                    @csrf
                                        ...
                                        <td class="py-3 text-right">
                                            <form method="POST" action="{{ route('superadmin.billing.certificates.destroy', $cert->id) }}" onsubmit="return confirm('¿Estás seguro de eliminar esta firma de la plataforma?');" class="inline">
                                                @csrf
                                                @method('DELETE')
            ...
            {{-- Formulario para Subir Nueva Firma de la Plataforma --}}
            <div class="card-panel p-6 space-y-4">
                <h3 class="text-sm font-bold border-b pb-2" style="color: var(--text-main); border-color: var(--border-light);">Subir Nueva Firma Electrónica (Plataforma)</h3>

                <form method="POST" action="{{ route('superadmin.billing.store') }}" enctype="multipart/form-data" class="space-y-4">
                    @csrf
                    ...
                    <div class="flex justify-end pt-2">
                        <button type="submit" class="btn-primary py-1.5 px-4 text-xs">
                            <i class="fa-solid fa-upload mr-1"></i> Cargar Firma Plataforma
                        </button>
                    </div>
                </form>
            </div>
            ...
            <div class="card-panel p-6">
                <form method="POST" action="{{ route('superadmin.billing.store') }}" class="space-y-6">
                    @csrf
                    
                    <h3 class="text-sm font-bold border-b pb-2" style="color: var(--text-main); border-color: var(--border-light);">Parámetros de Emisión (SaaS)</h3>
