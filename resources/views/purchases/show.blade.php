...
@section('title', 'Compra ' . $purchase->series . '-' . $purchase->number)
@section('page-title', 'Detalle de Comprobante de Compra')
@section('page-subtitle', 'Consulta de estados, ingreso de mercadería e historial de pagos')
...
@section('content')
    <div class="mt-2 space-y-6 page-fade">
            ...
            <div class="flex flex-wrap gap-2">
                ...
                @if ($purchase->status === 'BORRADOR')
                    ...
                    <form method="POST" action="{{ route('purchases.approve', $purchase) }}" class="inline"
                        onsubmit="return confirm('¿Aprobar esta compra? Se ingresará stock al inventario y el documento quedará congelado.')">
                        @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div class="space-y-2 md:border-r pr-6" style="border-color: var(--border-light);">
                    ...
                    <div class="pt-2">
                        <span class="text-xs block uppercase font-bold" style="color: var(--text-tertiary);">Tipo
                            Transacción</span>
                        <span class="font-bold" style="color: var(--text-main);">INGRESO POR COMPRA</span>
                    </div>
                    <div class="grid grid-cols-2 gap-2">
                        <div>
                            <span class="text-xs block uppercase font-bold" style="color: var(--text-tertiary);">Número
                                Interno</span>
                            <span class="font-mono text-sm font-bold"
                                style="color: var(--text-main);">{{ $purchase->series }}-{{ $purchase->number }}</span>
                        </div>
                        <div>
                            <span class="text-xs block uppercase font-bold" style="color: var(--text-tertiary);">Doc.
                                Proveedor</span>
                            <span class="font-mono text-sm font-bold"
                                style="color: var(--primary);">{{ $purchase->supplier_invoice ?? 'Sin asociar' }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-2 md:border-r pr-6" style="border-color: var(--border-light);">
                    <div>
                        ...
                        <span class="font-bold"
                            style="color: var(--text-secondary);">{{ $purchase->issue_date->format('d/m/Y') }}</span>
                    </div>
                    <div>
                        <span class="text-xs block uppercase font-bold" style="color: var(--text-tertiary);">Vencimiento
                            Compra</span>
                        <span class="font-bold"
                            style="color: var(--text-secondary);">{{ $purchase->due_date ? $purchase->due_date->format('d/m/Y') : 'Al contado' }}</span>
                    </div>
                    <div>
                        ...
                        @switch($purchase->status)
                            ...
                            @case('ANULADO')
                                <span class="badge badge-danger"><i class="fa-solid fa-circle-xmark"></i> Anulado (Stock
                                    Devuelto)</span>
                            @break
                        @endswitch
                    </div>
                ...
                <div class="space-y-2">
                    <span class="text-xs block uppercase font-bold" style="color: var(--text-tertiary);">Datos del
                        Proveedor</span>
                    <div class="font-bold" style="color: var(--text-main);">{{ $purchase->partner->business_name }}</div>
                    <div class="text-xs font-mono" style="color: var(--text-tertiary);">
                        {{ $purchase->partner->document_type }}: {{ $purchase->partner->document_number }}</div>
                    @if ($purchase->partner->phone)
                        <div class="text-xs" style="color: var(--text-secondary);">Tel: {{ $purchase->partner->phone }}
                        </div>
                    @endif
                    @if ($purchase->partner->address)
                        <div class="text-xs" style="color: var(--text-tertiary);">{{ $purchase->partner->address }}
                            {{ $purchase->partner->city ? '(' . $purchase->partner->city . ')' : '' }}</div>
                    @endif
                </div>
            </div>
        </div>
        ...
        <div class="card-panel overflow-hidden">
            <div class="px-6 py-4 border-b bg-slate-50"
                ...
                <h3 class="font-bold text-xs uppercase tracking-wider" style="color: var(--text-main);">Artículos Facturados
                </h3>
            </div>
            <table class="table-custom w-full">
                <thead>
                    <tr>
                        ...
                        <th class="table-header">Descripción Artículo</th>
                        <th class="table-header text-right">Cantidad</th>
                        <th class="table-header text-right">Costo Unitario</th>
                        <th class="table-header text-right">Desc. Línea</th>
                        <th class="table-header text-right">Subtotal Neto</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach ($purchase->details as $detail)
                        <tr>
                            ...
                            <td class="table-cell text-right font-semibold font-mono"
                                style="color: var(--text-secondary);">S/ {{ number_format($detail->unit_cost, 2) }}</td>
            ...
            <div class="border-t p-6 flex justify-end"
                ...
                <div class="w-80 space-y-2 text-sm" style="color: var(--text-secondary);">
                    ...
                    <div class="flex justify-between">
                        <span>Impuesto ({{ $purchase->tax->name ?? 'Exento' }}
                            {{ $purchase->tax ? number_format($purchase->tax->rate, 0) . '%' : '' }}):</span>
                        <span class="font-bold font-mono" style="color: var(--text-main);">S/
                    ...
                    <div class="border-t pt-2 space-y-1.5 text-xs" style="border-color: var(--border-light);">
                        <div class="flex justify-between" style="color: var(--success);">
                            ...
                            <span class="font-bold font-mono">S/ {{ number_format($purchase->paid_amount, 2) }}</span>
                        </div>
                        <div class="flex justify-between {{ $purchase->pending_balance > 0 ? 'font-bold' : '' }}"
                            style="color: {{ $purchase->pending_balance > 0 ? 'var(--danger)' : 'var(--text-tertiary)' }}">
                            <span>Saldo por Pagar:</span>
                            <span class="font-mono">S/ {{ number_format($purchase->pending_balance, 2) }}</span>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <tbody>
                        @forelse($purchase->payments as $pay)
                            <tr>
                                ...
                                <td class="table-cell text-right font-bold font-mono" style="color: var(--danger);">S/
                                    {{ number_format($pay->amount, 2) }}</td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="5" class="table-cell text-center" style="color: var(--text-tertiary);">
                                    Ningún egreso/pago registrado en esta compra.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
    ...
    <div id="payment-modal" class="fixed inset-0 hidden flex items-center justify-center z-50"
        ...
        <div class="card-panel w-full max-w-md overflow-hidden p-0" style="border-radius: 20px;">
            <div class="flex items-center justify-between px-6 py-5 border-b" style="border-color: var(--border-light);">
                <h3 class="text-base font-bold" style="color: var(--text-main);">Registrar Pago a Proveedor</h3>
            ...
            <form method="POST" action="{{ route('purchases.payments.store', $purchase) }}" class="p-6 space-y-4">
                @csrf
                <div>
                ...
                <div>
                    <label for="amount" class="form-label">Monto de Pago (S/)</label>
                    <input type="number" name="amount" id="amount" step="0.01" min="0.01"
                        max="{{ $purchase->pending_balance }}" value="{{ $purchase->pending_balance }}"
                        class="input-solid font-bold" required>
                    <p class="text-xs mt-1" style="color: var(--text-tertiary);">Saldo pendiente máximo: S/
                        {{ number_format($purchase->pending_balance, 2) }}</p>
                </div>
                ...
                <div class="border-t pt-4 flex justify-end gap-2" style="border-color: var(--border-light);">
                    ...
                    <button type="submit" class="btn-primary">Guardar Egreso</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div id="cancel-modal" class="fixed inset-0 hidden flex items-center justify-center z-50"
        ...
        <div class="card-panel w-full max-w-md overflow-hidden p-0" style="border-radius: 20px;">
            <div class="flex items-center justify-between px-6 py-5 border-b" style="border-color: var(--border-light);">
                <h3 class="text-base font-bold" style="color: var(--danger);">Anular Comprobante de Compra</h3>
                <button onclick="closeCancelModal()" class="btn-icon">
                    <i class="fa-solid fa-xmark"></i>
                </button>
            </div>

            <form method="POST" action="{{ route('purchases.cancel', $purchase) }}" class="p-6 space-y-4"
                onsubmit="return confirm('¿Seguro que deseas anular esta compra? El stock ingresado se retirará y esta operación es irreversible.')">
                @csrf
                <div>
                    <label for="reason" class="form-label">Motivo de Anulación <span
                            class="text-red-500">*</span></label>
                    <textarea name="reason" id="reason" rows="3" class="input-solid"
                        placeholder="Ej: Mercadería devuelta por daños, error de facturación del proveedor, etc." required></textarea>
                </div>
