...
@section('page-title')
    <a href="{{ route('superadmin.dashboard') }}"
        class="text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 transition-colors mr-2">
        <i class="fa-solid fa-arrow-left text-base"></i>
    </a>
    Detalle de Empresa: {{ $company->name }}
@endsection
@section('page-subtitle', 'Monitoreo de usuarios activos, plan y registro histórico de pagos')
...
@section('content')
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        ...
        <div class="card-panel p-6 lg:col-span-2">
            <h3
                class="font-bold text-slate-800 dark:text-slate-100 border-b border-slate-200 dark:border-slate-800 pb-3 mb-4 flex items-center gap-2">
                <i class="fa-solid fa-credit-card text-brand-blue dark:text-primary"></i> Información de Suscripción
            </h3>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-6 text-sm">
                <div>
                    <span class="text-slate-400 block font-semibold text-xs uppercase tracking-wider mb-1">Plan
                        Actual</span>
                    <div class="flex items-center gap-3">
                        <span class="text-lg font-bold text-slate-800 dark:text-slate-100 capitalize">
                            Plan {{ $company->subscription_plan }}
                        </span>
                        <form action="{{ route('superadmin.companies.change-plan', $company) }}" method="POST"
                            class="inline">
                            @csrf
                            <select name="plan" onchange="this.form.submit()"
                                ...
                                <option value="basic" {{ $company->subscription_plan === 'basic' ? 'selected' : '' }}>Basic
                                </option>
                                <option value="standard" {{ $company->subscription_plan === 'standard' ? 'selected' : '' }}>
                                    Standard</option>
                            </select>
                        </form>
                    </div>
                ...
                <div>
                    <span class="text-slate-400 block font-semibold text-xs uppercase tracking-wider mb-1">Estado de
                        Suscripción</span>
                    <div>
                        @if ($company->subscription_status === 'active')
                            <span
                                class="px-2.5 py-1 rounded-full text-xs font-semibold bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-300">
                                Activa
                            </span>
                        @elseif($company->subscription_status === 'pending_approval')
                            <span
                                class="px-2.5 py-1 rounded-full text-xs font-semibold bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-300">
                                Pendiente de Aprobación
                            </span>
                        @elseif($company->subscription_status === 'rejected')
                            <span
                                class="px-2.5 py-1 rounded-full text-xs font-semibold bg-rose-150 text-rose-800 dark:bg-rose-900/30 dark:text-rose-350">
                                Rechazada
                            </span>
                        @else
                            <span
                                class="px-2.5 py-1 rounded-full text-xs font-semibold bg-rose-100 text-rose-800 dark:bg-rose-900/30 dark:text-rose-300">
                                Inactiva
                            </span>
                        @endif
                    </div>
                ...
                <div>
                    <span class="text-slate-400 block font-semibold text-xs uppercase tracking-wider mb-1">Fecha de
                        Inicio</span>
                ...
                <div>
                    <span class="text-slate-400 block font-semibold text-xs uppercase tracking-wider mb-1">Fecha de Fin
                        (Vencimiento)</span>
                    <span class="text-slate-800 dark:text-slate-200 font-semibold flex items-center gap-1.5">
                        ...
                        @if ($company->subscription_expires_at)
                            ...
                            @else
                                <span
                                    class="text-xs text-slate-400">({{ now()->diffInDays($company->subscription_expires_at) }}
                                    dÃ­as restantes)</span>
                            @endif
                        @endif
                    </span>
                </div>
            ...
            <div class="mt-6 pt-4 border-t border-slate-200 dark:border-slate-800 flex justify-end gap-3">
                <form action="{{ route('superadmin.companies.toggle-status', $company) }}" method="POST" class="inline"
                    @if ($company->subscription_status === 'active') onsubmit="const reason = prompt('Motivo de desactivación:'); if (reason === null) return false; this.reason.value = reason;" @endif>
                    @csrf
                    <input type="hidden" name="reason" value="">
                    <button type="submit" class="btn-secondary text-sm py-2 px-4 inline-flex items-center gap-2">
                        <i class="fa-solid fa-power-off text-xs"></i>
                        {{ $company->subscription_status === 'active' ? 'Suspender Suscripción' : 'Activar Suscripción' }}
                    </button>
                </form>
            </div>
        </div>
        ...
        <div class="card-panel p-6">
            ...
            <div class="space-y-3.5 text-sm">
                <div>
                    <span class="text-slate-400 block font-semibold text-xs uppercase tracking-wider">Razón Social</span>
                    <span class="font-semibold text-slate-800 dark:text-slate-150">{{ $company->name }}</span>
                </div>
                <div>
                    <span class="text-slate-400 block font-semibold text-xs uppercase tracking-wider">Tipo de
                        Contribuyente</span>
                    <span
                        class="capitalize text-slate-700 dark:text-slate-300">{{ str_replace('_', ' ', $company->company_type) }}</span>
                </div>
                @if ($company->tax_id)
                    <div>
                        <span class="text-slate-400 block font-semibold text-xs uppercase tracking-wider">Identificación
                            Tributaria (RUC/NIT)</span>
                        <span class="font-mono text-slate-700 dark:text-slate-300">{{ $company->tax_id }}</span>
                    </div>
                @endif
                <div>
                    <span class="text-slate-400 block font-semibold text-xs uppercase tracking-wider">Dirección</span>
                    <span class="text-slate-700 dark:text-slate-300">
                        {{ $company->address ?? ($company->legal_address ?? 'No registrada') }}, {{ $company->city }},
                        {{ $company->country }}
                    </span>
                </div>
            </div>
    ...
    <div class="card-panel p-6 mb-6">
        <h3
            class="font-bold text-slate-800 dark:text-slate-100 border-b border-slate-200 dark:border-slate-800 pb-3 mb-4 flex items-center gap-2">
            <i class="fa-solid fa-sliders text-brand-blue dark:text-primary"></i> Personalización de Suscripción (Overrides)
        </h3>
        <form action="{{ route('superadmin.companies.custom-limits', $company) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                ...
                <div class="p-4 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
                    <label
                        class="flex items-center gap-2 font-semibold text-sm text-slate-700 dark:text-slate-200 mb-2 cursor-pointer">
                        <input type="checkbox" name="override_admins" value="1"
                            {{ !is_null($company->getRawOriginal('max_admins')) ? 'checked' : '' }}
                    ...
                    <input type="number" name="max_admins" id="max_admins_input" value="{{ $company->max_admins }}"
                        {{ is_null($company->getRawOriginal('max_admins')) ? 'disabled' : '' }}
                        class="input-solid text-xs py-1.5" min="1">
                    <span class="text-[10px] text-slate-400 block mt-1">Por defecto del plan:
                        {{ $company->plan?->max_admins ?? 1 }}</span>
                </div>
                ...
                <div class="p-4 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
                    <label
                        class="flex items-center gap-2 font-semibold text-sm text-slate-700 dark:text-slate-200 mb-2 cursor-pointer">
                        <input type="checkbox" name="override_sellers" value="1"
                            {{ !is_null($company->getRawOriginal('max_sellers')) ? 'checked' : '' }}
                    ...
                    <input type="number" name="max_sellers" id="max_sellers_input" value="{{ $company->max_sellers }}"
                        {{ is_null($company->getRawOriginal('max_sellers')) ? 'disabled' : '' }}
                        class="input-solid text-xs py-1.5" min="0">
                    <span class="text-[10px] text-slate-400 block mt-1">Por defecto del plan:
                        {{ $company->plan?->max_sellers ?? 2 }}</span>
                </div>
                ...
                <div class="p-4 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
                    <label
                        class="flex items-center gap-2 font-semibold text-sm text-slate-700 dark:text-slate-200 mb-2 cursor-pointer">
                        <input type="checkbox" name="override_transactions" value="1"
                            {{ !is_null($company->getRawOriginal('max_monthly_transactions')) ? 'checked' : '' }}
                    ...
                    <input type="number" name="max_monthly_transactions" id="max_transactions_input"
                        ...
                        class="input-solid text-xs py-1.5" min="1">
                    <span class="text-[10px] text-slate-400 block mt-1">Por defecto del plan:
                        {{ $company->plan?->max_monthly_transactions ?? 100 }}</span>
                </div>
            </div>
            ...
            <div class="p-5 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 mb-6">
                <label
                    class="flex items-center gap-2 font-bold text-sm text-slate-700 dark:text-slate-200 mb-4 cursor-pointer">
                    <input type="checkbox" name="override_modules" value="1" id="override_modules_checkbox"
                        ...
                        class="rounded border-slate-350 dark:border-slate-700 text-brand-blue">
                    <span>Personalizar Módulos Habilitados (Ignorar por defecto del plan)</span>
                </label>
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    @foreach ($availableModules as $code => $label)
                        <label
                            class="flex items-center gap-2 text-xs font-semibold cursor-pointer text-slate-700 dark:text-slate-300">
            ...
            <div class="flex justify-end">
                <button type="submit" class="btn-primary text-sm py-2 px-5">
                    <i class="fa-solid fa-floppy-disk mr-1"></i> Guardar Personalización
                </button>
            </div>
        </form>
    </div>
        ...
        <div class="card-panel overflow-hidden">
            <div class="p-6 border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50">
                <h3 class="font-bold text-slate-800 dark:text-slate-100 flex items-center justify-between">
                    ...
                    <span
                        class="px-2 py-0.5 rounded text-xs bg-indigo-100 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 font-bold">
                        {{ $activeAdmins->count() }}
                    </span>
                </h3>
            </div>
            <div class="overflow-x-auto">
                <table class="w-full text-left text-sm whitespace-nowrap">
                    <thead>
                        <tr
                            class="bg-slate-50 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 text-slate-400 font-bold text-xs uppercase">
                    ...
                    <tbody class="divide-y divide-slate-200 dark:divide-slate-800">
                        @forelse($activeAdmins as $admin)
                            <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-900/50 transition-colors">
                                <td class="px-6 py-3 font-semibold text-slate-800 dark:text-slate-250">{{ $admin->name }}
                                </td>
                                <td class="px-6 py-3 text-slate-500">{{ $admin->email }}</td>
                                <td class="px-6 py-3 capitalize text-xs font-semibold text-indigo-500">
                                    {{ str_replace('_', ' ', $admin->role) }}</td>
                            </tr>
                        @empty
                            <tr>
        ...
        <div class="card-panel overflow-hidden">
            <div class="p-6 border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50">
                <h3 class="font-bold text-slate-800 dark:text-slate-100 flex items-center justify-between">
                    ...
                    <span
                        class="px-2 py-0.5 rounded text-xs bg-teal-100 dark:bg-teal-900/30 text-teal-700 dark:text-teal-300 font-bold">
                        {{ $activeSellers->count() }}
                    </span>
                </h3>
            </div>
                ...
                <table class="w-full text-left text-sm whitespace-nowrap">
                    ...
                    <tbody class="divide-y divide-slate-200 dark:divide-slate-800">
                        @forelse($activeSellers as $seller)
                            <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-900/50 transition-colors">
                                <td class="px-6 py-3 font-semibold text-slate-800 dark:text-slate-250">{{ $seller->name }}
                                </td>
    ...
    <div class="card-panel overflow-hidden">
        <div class="p-6 border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50">
            <h3 class="font-bold text-slate-800 dark:text-slate-100 flex items-center gap-2">
                <i class="fa-solid fa-receipt text-amber-500"></i> Registro Histórico de Pagos
            </h3>
        </div>
        <div class="overflow-x-auto">
            <table class="w-full text-left text-sm whitespace-nowrap">
                    ...
                    @forelse($company->subscriptionPayments as $payment)
                        <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-900/50 transition-colors">
                            ...
                            <td class="px-6 py-4">
                                ...
                                @else
                                    <span
                                        class="text-xs font-bold uppercase tracking-wider text-emerald-500">Renovación</span>
                                @endif
                            </td>
                            <td class="px-6 py-4 capitalize text-slate-700 dark:text-slate-300 font-semibold">
                                {{ $payment->plan }}</td>
                            ...
                            <td class="px-6 py-4">
                                @if ($payment->status === 'approved')
                                    <span
                                        class="px-2 py-0.5 rounded text-xs font-semibold bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-300">
                                        Aprobado
                                    </span>
                                @elseif($payment->status === 'rejected')
                                    <span
                                        class="px-2 py-0.5 rounded text-xs font-semibold bg-rose-100 text-rose-800 dark:bg-rose-900/30 dark:text-rose-300">
                                        Rechazado
                                    </span>
                                @else
                                    <span
                                        class="px-2 py-0.5 rounded text-xs font-semibold bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-300">
                                        Pendiente
                                    </span>
                                @endif
                            </td>
                            <td class="px-6 py-4 text-center">
                                @if ($payment->status === 'pending')
                                    <div class="flex items-center justify-center gap-2">
                                        <form action="{{ route('superadmin.companies.approve', $company) }}"
                                        ...
                                        <form action="{{ route('superadmin.companies.reject', $company) }}"
                                            method="POST" class="inline"
